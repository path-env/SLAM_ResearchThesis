...
def ROS_bag_run(SM_meths, Particle_N, AccErr):
    iterate = 0
    ...
    analysis = aly(f'GMapping-{Methods[SM_meths]}-{Particle_N} particles({AccErr})')
    # GMAP = Map()
    slam_obj = Gmapping(analysis, SM_meths, Particle_N, AccErr)
    ...
    for topic, msg, t in bag.read_messages(topics=['/carla/ego_vehicle/gnss/gnss1/fix',
        ...
        if  topic == '/carla/ego_vehicle/vehicle_status': 
            ...
            Meas_X_t['steer'] = np.rad2deg(msg.control.steer * max_steering_angle) # on a scale [-1,1] of max_steering_angle
            Vel_avail =1
        ...
        if Imu_avail and Gps_avail and Odom_avail and Lidar_avail and Vel_avail and veh_info: 
            # writer.writerow([Meas_X_t['t'], Meas_X_t['x'],Meas_X_t['y'], Meas_X_t['yaw'], Meas_X_t['v'], Meas_X_t['acc'], Meas_X_t['steer'], GPS_Z_t[2],
            #     GPS_Z_t[1], GPS_Z_t[0], IMU_Z_t['roll'], IMU_Z_t['pitch'],  IMU_Z_t['yaw'] ,IMU_Z_t['ang_vel'] ])
            if (Meas_X_t['v']>0.1 or Meas_X_t['v'] <-0.1):
    ...
    analysis.error_plot(Particle_N,str(AccErr))
    analysis.analysis_metrics()
    analysis.benchmark_metric()
    # MM_size = np.size(np.where(slam_obj.model_used[0]==1))/slam_obj.particleCnt
    # SM_size = np.size(np.where(slam_obj.model_used[1]==2))/slam_obj.particleCnt
...
if __name__ == '__main__':
    SM_Method = [0,1,2]
    Particle_N = [20]
    AccErr = {'0':[0.001,0.0015],'1':[0.001,0.0015],'2':[1/0.05,1/0.052]}
    # ROS_bag_run(1, 10, 0.001)
    for SM  in SM_Method:
        for P in Particle_N:
            for err in AccErr[str(SM)]:
                print('#####################################################')
                start = time.perf_counter()
                ROS_bag_run(SM, P, err)
                stop = time.perf_counter()
                print(f'Time to complete {Methods[SM]} using {P} particles with accepted error of {err} is :{stop - start} seconds(s)')
                print('#####################################################')
